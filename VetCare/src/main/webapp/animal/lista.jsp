...
<%
    ...
    boolean podeEditar = isGerente; // Só gerente pode editar
    
    String clienteNifFiltro = request.getParameter("clienteNif");
%>
<!DOCTYPE html>
<html>
<head>
    ...
    <div class="header">
        <div class="container">
            ...
            <nav>
                ...
                <% } %>
                <a href="${pageContext.request.contextPath}/animais<%= isCliente ? "?clienteNif=" + user.getNifCliente() : "" %>" class="nav-link">Animais</a>
                <a href="${pageContext.request.contextPath}/agendamentos<%= isCliente ?  "?clienteNif=" + user.getNifCliente() : "" %>" class="nav-link">Agendamentos</a>
                <% if (isGerente) { %>
    ...
    <div class="container">
        <div class="card">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom:20px;">
                <h2><%= isCliente ? "Os Meus Animais" : "Lista de Animais" %></h2>
            ...
            <% if (isVeterinario) { %>
                <div style="background:#ebf8ff; border: 1px solid #4299e1; padding:10px; border-radius:5px; margin-bottom:15px;">
                    <small>ℹ️ Modo de consulta - não é possível editar dados de animais. </small>
                </div>
            <% } %>
            ...
            <table>
                ...
                <tbody>
                    ...
                    // Filtrar animais para cliente
                    if (lista != null) {
                        for (Animal a : lista) {
                            if (isCliente) {
                                // Cliente só vê os seus animais
                                if (a.getClienteNif() != null && a.getClienteNif().equals(user.getNifCliente())) {
                                    listaFiltrada.add(a);
                                }
                            } else {
                                listaFiltrada.add(a);
                    ...
                    %>
                     <tr>
                        <td style="width:50px; text-align:center;">
                            ...
                            <% } else { %>
                                <div style="width:40px; height: 40px; border-radius:50%; background:#eee; display:flex; align-items:center; justify-content:center; font-size:10px; color:#999;">Sem foto</div>
                            <% } %>
                        </td>
                        <td><%= a.getNome() %></td>
                    ...
                    %>
                    <tr><td colspan="<%= isCliente ? 5 : 6 %>">Sem registos. </td></tr>
                    <% } %>
                </tbody>
            </table>
        </div>
